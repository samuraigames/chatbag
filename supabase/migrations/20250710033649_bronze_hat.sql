/*
  ...
  1. New Tables
    ...
    - `chat_settings` - Per-user chat notification settings
    - `message_threads` - Support for threaded conversations
  ...
  2. Enhanced Existing Tables
    ...
    - Add edit/delete timestamps to messages
    - Add group chat metadata (name, description, avatar)
  ...
  4. Performance
    ...
    - Create optimized views for chat lists
...
BEGIN
  ...
  LOOP
    IF participant_id != NEW.sender_id THEN
      INSERT INTO message_status (message_id, user_id, status, updated_at)
      VALUES (NEW.id, participant_id, 'delivered', now())
...
DROP TRIGGER IF EXISTS trigger_new_message_status ON messages;
...
DROP TRIGGER IF EXISTS trigger_user_activity_messages ON messages;

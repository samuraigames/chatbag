/*
  ...
  1. New Tables
    ...
    - `message_attachments` - Store file attachments for messages
    - `user_presence` - Track user online/offline status
    - `chat_settings` - Per-user chat preferences
    - `message_threads` - Support for message replies
    - `user_blocks` - User blocking functionality
    - `message_status` - Track message delivery/read status

  2. Enhanced Tables
    - Add reply, edit, and delete functionality to messages
    - Add group chat naming and avatars
...
-- Create enum types for new features (with proper error handling)
DO $$ BEGIN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'message_reactions' AND policyname = 'Users can read reactions in their chats'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_reactions' AND policyname = 'Users can add reactions to messages in their chats'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_reactions' AND policyname = 'Users can remove their own reactions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_attachments' AND policyname = 'Users can read attachments in their chats'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_attachments' AND policyname = 'Users can add attachments to their messages'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_presence' AND policyname = 'Users can read all presence status'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_presence' AND policyname = 'Users can update their own presence'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'chat_settings' AND policyname = 'Users can manage their own chat settings'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_threads' AND policyname = 'Users can read threads in their chats'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_threads' AND policyname = 'Users can create threads in their chats'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_blocks' AND policyname = 'Users can manage their own blocks'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_status' AND policyname = 'Users can read message status in their chats'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'message_status' AND policyname = 'Users can update message status for themselves'
  ) THEN
...
-- Trigger to update message status when message is inserted
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tgname = 'trigger_new_message_status'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'trigger_user_activity_messages'
  ) THEN

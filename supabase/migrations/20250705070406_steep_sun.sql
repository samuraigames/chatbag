/*
  # Comprehensive Chat App Features

  1. New Tables
    - `message_reactions` - For emoji reactions to messages
    - `message_attachments` - For file/image attachments
    - `user_presence` - For online/offline status
    - `chat_settings` - For chat-specific settings
    - `message_threads` - For threaded conversations
...
CREATE TYPE IF NOT EXISTS reaction_type AS ENUM ('👍', '👎', '❤️', '😂', '😮', '😢', '😡');
CREATE TYPE IF NOT EXISTS attachment_type AS ENUM ('image', 'file', 'video', 'audio');
CREATE TYPE IF NOT EXISTS presence_status AS ENUM ('online', 'offline', 'away', 'busy');
